        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('aktivitaet_zeitpunkt')}>
                <span className="inline-flex items-center gap-1">
                  Datum und Uhrzeit
                  {sortKey === 'aktivitaet_zeitpunkt' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('aktivitaet_beschreibung')}>
                <span className="inline-flex items-center gap-1">
                  Aktivitätsbeschreibung
                  {sortKey === 'aktivitaet_beschreibung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('kalorien_verbrannt')}>
                <span className="inline-flex items-center gap-1">
                  Verbrannte Kalorien (kcal)
                  {sortKey === 'kalorien_verbrannt' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('durchschnittspuls')}>
                <span className="inline-flex items-center gap-1">
                  Durchschnittlicher Puls (Schläge/min)
                  {sortKey === 'durchschnittspuls' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.aktivitaet_zeitpunkt)}</TableCell>
                ...
                <TableCell className="font-medium">{record.fields.aktivitaet_beschreibung ?? '—'}</TableCell>
                <TableCell>{record.fields.dauer_minuten ?? '—'}</TableCell>
                <TableCell>{record.fields.distanz_km ?? '—'}</TableCell>
                <TableCell>{record.fields.kalorien_verbrannt ?? '—'}</TableCell>
                ...
                <TableCell>{record.fields.durchschnittspuls ?? '—'}</TableCell>
